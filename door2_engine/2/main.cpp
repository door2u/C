...
// renamed and moved Rota / RotaVert
// rend function
// object = functions werent quite right
// objestat new structs

// DO LATER: reorganize object folder to one object per header
// assign color to vertices
// organize
// make sure unecessary includes are gone
// make sure functions have dependencies declared even if theyre above

// adding Plan structure

// make order functions, brute and by region

// use short int if possible

// Orde:
// make a bool list parallel to planList
// when the camera enters a new region, update ^^^
// make a ""pair"" list, increment when a plane is true
// if the pair list gets to "6", move a poly in front of another. if the pair list gets to "-6", move the other.

// maybe clear border planes from planList at the end
...
int main(int argc, char **argv) {
	...
	//std::cout << objeStatList.size() << std::endl;
	//std::cout << objeStatList[0].vertList.size() << std::endl;
...
void Move(unsigned int widt, unsigned int heig, Came *came, bool keyw, bool keya, bool keys, bool keyd, bool keyq, bool keye, int difx, int dify, float cameSpee, Display *disp, Window *wind, int soux, int souy, bool warpPoin) {
	...
	if (difx != 0) {
		...
		// TODO
		//Rota(&came->u, mo_x, moux);
		//Rota(&came->w, mo_x, moux);
		//Rota(&came->v, mo_x, moux);
	...
	if (dify != 0) {
		...
		// TODO
		//Rota(&camwNext, mo_y, came->u);
		//Rota(&camvNext, mo_y, came->u);
