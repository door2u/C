...
int main(int argc, char **argv)
{
	...
	// window resolution
	unsigned int widt = 640;
	unsigned int heig = 480;
	// background color
	float skyr = 0.6;
	float skyg = 0.8;
	float skyb = 1.0;
	...
	void Rend(Display *disp, Window *wind, float skyr, float skyg, float skyb, float skya);
	...
	while (runn) {
		if (XPending(disp)) {
			...
			if (timeElap.count() > timeFram) {
				...
				// refresh gl / x11
				Rend(disp, &wind, skyr, skyg, skyb, skya);
...
void Inpu(unsigned int widt, unsigned int heig, Display *disp, XEvent *even, bool *keyw, bool *keya, bool *keys, bool *keyd, bool *keyq, bool *keye, int *difx, int *dify, bool *runn) {
	...
	if (even->type == KeyPress && XLookupString(&even->xkey, keybInpu, 255, &key, 0) == 1) {
		...
		if (keybInpu[0] == 'w') *keyw = true; if (keybInpu[0] == 'a') *keya = true; if (keybInpu[0] == 's') *keys = true; if (keybInpu[0] == 'd') *keyd = true; if (keybInpu[0] == 'q') *keyq = true; if (keybInpu[0] == 'e') *keye = true;
	}
	if (even->type == KeyRelease && XLookupString(&even->xkey, keybInpu, 255, &key, 0) == 1) {
		if (keybInpu[0] == 'w') *keyw = false; if (keybInpu[0] == 'a') *keya = false; if (keybInpu[0] == 's') *keys = false; if (keybInpu[0] == 'd') *keyd = false; if (keybInpu[0] == 'q') *keyq = false; if (keybInpu[0] == 'e') *keye = false;
	}
}
...
void Move(unsigned int widt, unsigned int heig, Came *came, bool keyw, bool keya, bool keys, bool keyd, bool keyq, bool keye, int difx, int dify, float cameSpee, Display *disp, Window *wind, int soux, int souy, bool warpPoin) {
	...
	if (dify != 0) {
		...
		Vec3Clas camwNext = came->w;
		Vec3Clas camvNext = came->v;
